...
AForm::AForm(const std::string &userdef_name, const std::string& userdef_target, const int execute, const int sign) : name(userdef_name),
             target(userdef_target), grade_execute(execute), grade_sign(sign)
{
    if(execute < 1 || sign < 1)
...
AForm::AForm(const AForm& other) : name(other.name), grade_execute(other.grade_execute), grade_sign(other.grade_sign){}
...
int AForm::getExecuteGrade() const {return grade_execute;}

int AForm::getSignGrade() const {return grade_sign;}
